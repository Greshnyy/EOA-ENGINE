...
Editor::~Editor() { if (device_ != VK_NULL_HANDLE) vkDeviceWaitIdle(device_); ImGui_ImplVulkan_Shutdown(); ImGui_ImplGlfw_Shutdown(); if (ImGui::GetCurrentContext()) ImGui::DestroyContext(); if (imGuiDescriptorPool_ != VK_NULL_HANDLE) { vkDestroyDescriptorPool(device_, imGuiDescriptorPool_, nullptr); imGuiDescriptorPool_ = VK_NULL_HANDLE; } }
void Editor::InitImGui(VkInstance inst, VkPhysicalDevice phys, VkDevice dev, uint32_t gfxFam, VkQueue gfxQ, VkRenderPass rp, uint32_t imgCnt) { IMGUI_CHECKVERSION(); ImGui::CreateContext(); ImGuiIO& io = ImGui::GetIO(); io.ConfigFlags |= ImGuiConfigFlags_DockingEnable; io.IniFilename = nullptr; ImGui::StyleColorsDark(); ImGuiStyle& style = ImGui::GetStyle(); style.WindowRounding=3.0f; style.FrameRounding=2.0f; style.WindowPadding=ImVec2(5,5); style.FramePadding=ImVec2(6,3); style.ItemSpacing=ImVec2(5,4); ImGui_ImplGlfw_InitForVulkan(window_, true); CreateDescriptorPool(); ImGui_ImplVulkan_InitInfo ii{}; ii.Instance=inst; ii.PhysicalDevice=phys; ii.Device=dev; ii.QueueFamily=gfxFam; ii.Queue=gfxQ; ii.PipelineCache=VK_NULL_HANDLE; ii.DescriptorPool=imGuiDescriptorPool_; ii.MinImageCount=imgCnt; ii.ImageCount=imgCnt; ii.Allocator=nullptr; ii.CheckVkResultFn=CheckVkResult; ii.PipelineInfoMain.RenderPass=rp; ii.PipelineInfoMain.Subpass=0; ii.PipelineInfoMain.MSAASamples=VK_SAMPLE_COUNT_1_BIT; ImGui_ImplVulkan_Init(&ii); }
void Editor::CreateDescriptorPool() { VkDescriptorPoolSize sizes[]={{VK_DESCRIPTOR_TYPE_SAMPLER,1000},{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,1000},{VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,1000},{VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,1000},{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1000},{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,1000},{VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,1000}}; VkDescriptorPoolCreateInfo info{}; info.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO; info.flags=VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT; info.maxSets=7000; info.poolSizeCount=static_cast<uint32_t>(sizeof(sizes)/sizeof(sizes[0])); info.pPoolSizes=sizes; EOA_CHECK_VK(vkCreateDescriptorPool(device_,&info,nullptr,&imGuiDescriptorPool_)); }
void Editor::SetupDocking() { ImGuiID dock=ImGui::DockSpaceOverViewport(0,ImGui::GetMainViewport(),ImGuiDockNodeFlags_PassthruCentralNode); if(!firstFrame_) return; firstFrame_=false; ImGui::DockBuilderRemoveNode(dock); ImGui::DockBuilderAddNode(dock,ImGuiDockNodeFlags_DockSpace); ImGui::DockBuilderSetNodeSize(dock,ImGui::GetMainViewport()->Size); ImGuiID right=0,left=0,bottom=0; ImGui::DockBuilderSplitNode(dock,ImGuiDir_Right,0.24f,&right,&dock); ImGui::DockBuilderSplitNode(dock,ImGuiDir_Left,0.20f,&left,&dock); ImGui::DockBuilderSplitNode(dock,ImGuiDir_Down,0.24f,&bottom,&dock); ImGui::DockBuilderDockWindow("Project",left); ImGui::DockBuilderDockWindow("Hierarchy",left); ImGui::DockBuilderDockWindow("Inspector",right); ImGui::DockBuilderDockWindow("Content Browser",bottom); ImGui::DockBuilderDockWindow("Console",bottom); ImGui::DockBuilderDockWindow("Viewport",dock); ImGui::DockBuilderFinish(dock); }
...
void Editor::DrawViewportPanel(){ ImGui::Begin("Viewport",nullptr,ImGuiWindowFlags_NoScrollbar); ImVec2 pos=ImGui::GetCursorScreenPos(),size=ImGui::GetContentRegionAvail(); HandleViewportInput(size,pos); ImDrawList* dl=ImGui::GetWindowDrawList(); dl->AddRectFilled(pos,ImVec2(pos.x+size.x,pos.y+size.y),IM_COL32(18,20,22,255)); const float grid=64.0f; for(float x=std::fmod(-pos.x,grid);x<size.x;x+=grid) dl->AddLine(ImVec2(pos.x+x,pos.y),ImVec2(pos.x+x,pos.y+size.y),IM_COL32(48,48,52,120)); for(float y=std::fmod(-pos.y,grid);y<size.y;y+=grid) dl->AddLine(ImVec2(pos.x,pos.y+y),ImVec2(pos.x+size.x,pos.y+y),IM_COL32(48,48,52,120)); DrawParticles(dl,pos,size); ImGui::SetCursorScreenPos(pos); ImGui::TextColored(ImVec4(0.65f,0.65f,0.65f,1),"Viewport | RMB orbit | Wheel zoom | W/E/R mode"); ImGui::End(); }
void Editor::DrawHierarchyPanel(){ ImGui::Begin("Hierarchy"); World* world=externalWorld_?externalWorld_:&editorWorld_; if(ImGui::Button("+ Actor")) world->SpawnActor("Actor"); ImGui::Separator(); for(size_t i=0;i<world->actors.size();++i){Actor* actor=world->actors[i].get();if(!actor)continue;if(ImGui::Selectable(actor->GetName().c_str(),selectedObjectIndex_==static_cast<int>(i)))selectedObjectIndex_=static_cast<int>(i);} ImGui::End(); }
void Editor::DrawInspectorPanel(){ ImGui::Begin("Inspector"); World* world=externalWorld_?externalWorld_:&editorWorld_; if(selectedObjectIndex_>=0&&selectedObjectIndex_<static_cast<int>(world->actors.size())){Actor* actor=world->actors[selectedObjectIndex_].get();ImGui::Text("Actor: %s",actor->GetName().c_str());if(auto* t=actor->GetComponent<TransformComponent>()){glm::vec3 p=t->GetPosition(),s=t->GetScale();if(ImGui::DragFloat3("Position",glm::value_ptr(p),0.05f))t->SetPosition(p);if(ImGui::DragFloat3("Scale",glm::value_ptr(s),0.05f,0.001f,100.0f))t->SetScale(s);}}else ImGui::TextDisabled("No actor selected"); ImGui::End(); }
void Editor::DrawContentBrowser(){ ImGui::Begin("Content Browser"); if(ImGui::Button("Refresh"))RefreshAssetEntries();ImGui::SameLine();ImGui::TextUnformatted(currentAssetPath_.c_str());ImGui::Separator(); for(const auto& path:assetEntries_){ if(ImGui::Selectable(path.c_str())){std::string full=currentAssetPath_+path;if(HasExtension(full,{".gltf",".glb",".obj",".fbx"})&&onAssetActivated_)onAssetActivated_(full);} } ImGui::End(); }
...
void Editor::DrawParticlePanel(){ImGui::Begin("Particles");ImGui::Text("Emitters: %zu",particleSystem_.GetEmitters().size());ImGui::Text("Particles: %zu",particleSystem_.GetParticles().size());ImGui::End();}
...
void Editor::DrawParticles(ImDrawList* dl,const ImVec2& pos,const ImVec2& size){if(!dl||size.x<=0||size.y<=0||particleSystem_.GetParticles().empty())return;const auto& particles=particleSystem_.GetParticles();float cx=pos.x+size.x*0.5f,cy=pos.y+size.y*0.5f;for(size_t i=0;i<std::min<size_t>(particles.size(),128);++i){const auto& p=particles[i];if(!p.alive)continue;dl->AddCircleFilled(ImVec2(cx+p.position.x*25.0f,cy-p.position.y*25.0f),std::max(1.0f,p.size*8.0f),IM_COL32(220,180,90,180));}}
void Editor::DrawProfilerOverlay(){} void Editor::CheckShaderChanges(){} void Editor::DrawTextureViewer(){} std::string Editor::OpenFileDialog(const char*,const char*){return {};} void Editor::ImportModel(){} void Editor::ImportTexture(){}
} // namespace eoa
